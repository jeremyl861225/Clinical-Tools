...
## 延長輸注建議（僅 β-lactam；院內有明文建議者才填）

`eif` 為物件，缺省則藥卡不顯示此欄位。資料來源為台大醫院藥劑部
《β-lactam 類抗生素輸注時間建議》（文件 15600-3-000002，版次 3）表一——
**只有該表列到的藥才可以填**，其餘藥物留空，避免對沒有院內建議的藥物無中生有。

| 子欄位 | 型別 | 必要 | 說明 |
|---|---|---|---|
| `when` | string | 必要 | 什麼情況才延長輸注（MIC 門檻／免疫不全／重症）。缺此欄 validator 報錯——沒有它就成了「這支藥一律延長輸注」的無條件宣稱。 |
| `ld` | string | 選填 | Loading dose 與其輸注時間。 |
| `md` | `{k, v}[]` | 選填 | Maintenance dose，`k`＝CLCr 分層或 HD／PD／CKRT，`v`＝劑量。渲染為 `renal-tbl` 表格。 |
| `solvent` | string | 選填 | 溶劑、最高濃度與 25°C 安定時數（meropenem 的「D5W 不可做延長輸注」寫在這裡）。 |
| `note` | string | 選填 | 原文件的限制或疑義（如複方製劑的計量基準、原表區間重疊）。 |

渲染位置在「注射給藥指引」之後、「剝半／磨粉／管餵」之前；沿用藥卡既有的
`inj-tbl`／`renal-tbl` 兩個 class（`css/ui-sentence.css` Q8 段已收，不必另補造句設計樣式）。
