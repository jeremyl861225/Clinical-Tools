...
(function () {
  ...
  /* 心室源複合波（折線頂點：毫秒／毫伏）。寬度來自「每一段的斜率都慢」，
     ...
     PPV＝心室早期收縮。三者的共同特徵是「起始與終末的斜率都慢、中途帶頓挫（slur）」
     與「T 波與主波反向」——把頂點多切一刀讓斜坡分兩段，複合波才看得出來是寬的，
     只把總寬度拉長但斜率仍陡的話，畫出來還是像一根細尖峰。 */
  var PVT = [[0, 0], [30, 0.32], [54, 0.10], [92, -0.66], [126, -1.30], [164, -0.52], [196, 0]];  // QRS ≈ 196 ms（其後另接正向 T）
  /* PAI／PPV 把 ST-T 也一起寫進折線：心室源心搏的 R 波之後是「直接落下去、不回基線」的
     一段又深又寬的反向 ST-T，若拆成「QRS 回到基線 → 空一段 → 另一個 T」，複合波就會被
     切成一根細尖峰加一個獨立的丘，看起來又變窄了。用到這兩組時 T 波參數要給 t:0。 */
  var PAI = [[0, 0], [12, -0.06], [48, 1.52], [80, 0.60], [112, -0.20], [150, -0.58],
             [200, -0.70], [262, -0.44], [318, -0.10], [356, 0]];                       // 全長 ≈ 356 ms
  var PPV = [[0, 0], [14, -0.06], [52, 1.45], [84, 0.55], [118, -0.20], [156, -0.55],
             [206, -0.66], [266, -0.40], [320, -0.08], [354, 0]];                       // 全長 ≈ 354 ms
  ...
  SPEC.pvc = { f: function (t) {                             // 竇性＋單一 PVC＋完全代償間歇
      ...
      y += beat(t, 1560, { p: 0, pr: 0, poly: PPV, t: 0 });
      return y;
    } };
  ...
  SPEC.vt = { h: 34, base: 14, f: function (t) {
      var y = series(t, every(333, 17, 25), function (tt, t0) {
        return beat(tt, t0, { p: 0, pr: 0, poly: PVT, stD: 2, td: 132, t: 0.70 });
      });
      return y + series(t, every(700, 8, 210), function (tt, t0) { return gau(tt, t0, 100, 0.08); });
    } };
  /* 58 bpm：RR 1035 ms，慢而寬——「像 VT 但速率不快」的重點就在這個密度 */
  SPEC.aivr = { h: 32, base: 18, f: rhythm(1035, 6, 60, { p: 0, pr: 0, poly: PAI, t: 0 }) };
  ...
  /* 整段都是尖端扭轉：連續振盪（週期 245 ms）乘上慢包絡（每 1160 ms 換一次極性＝扭轉一圈）。
     包絡給初相位，讓描跡一開始就有振幅——這是「錄到發作中段」的樣子，而不是從零長出來。 */
  SPEC.tdp = { h: 36, base: 18, f: function (t) {
      var w = 2 * Math.PI / 245;
      var env = 1.18 * Math.sin(Math.PI * (t + 430) / 1160);
      return env * (0.78 * Math.sin(w * t) + 0.22 * Math.sin(2 * w * t + 0.7));
    } };
  ...
  /* ---- 猝死型態（暈厥八症候群）----
     這一組的共同特徵是「不是缺血、但會死人」，各自的畫法陷阱寫在每一行後面。 */

  /* ARVC：兩個特徵都在 QRS 的「尾巴」，而且必須畫成兩件不同的事——
     (1) S 波觸底後回升得很慢（終末激動時間，此處 66 ms），這要靠折線把回升段
         切成兩段緩坡；用三角波元件排會回升得太陡，看起來只是個普通的 rS。
     (2) epsilon 波是 QRS 結束「之後」才冒出來的低振幅小丘，中間要留一段等電位，
         否則會與 (1) 連成兩個小 R′，變得像 rSr′ 而不是 ARVC。 */
  var PARV = [[0, 0], [16, 0.28], [30, 0.10], [46, -0.85], [70, -0.55], [100, -0.12], [112, 0]];
  SPEC.arvc = { f: function (t) {
      return series(t, every(880, 4, 60), function (tt, t0) {
        return beat(tt, t0, { p: 0.12, pd: 95, pr: 160, poly: PARV, stD: 110, td: 180, t: -0.36 })
             + rnd(tt, t0 + 292, 45, 0.13);
      });
    } };

  /* 鈉離子通道阻斷（三環抗鬱劑等）：aVR 的 P 本來就倒置，重點在終末那一段
     又寬又鈍的向上 R′——用「小的向下起始 ＋ 一個寬圓鈍隆起」兩段合成。 */
  SPEC.nachan = { h: 32, base: 20, f: rhythm(600, 5, 40, {
      p: -0.06, pd: 90, pr: 150, qrs: [[30, -0.30], [112, 0.62, 'r']],
      stD: 40, td: 160, t: -0.12 }) };

  /* 肥厚型心肌病：dagger Q 的關鍵是「窄而深」（<40 ms 卻可達 R 波的三分之一），
     若照一般病理性 Q 波畫成寬 Q，看起來就變成陳舊性梗塞而不是 HCM。 */
  SPEC.hocm = { h: 46, base: 28, f: rhythm(800, 4, 60, {
      p: 0.14, pd: 130, pshape: 'bifid', pr: 175,
      qrs: [[18, -0.62], [50, 1.85], [30, -0.18]],
      stD: 60, st: -0.16, st2: -0.26, td: 170, t: -0.52 }) };
  SPEC.apicalhcm = { h: 48, base: 20, f: rhythm(850, 4, 60, {
      p: 0.14, pr: 165, qrs: [[20, -0.10], [52, 1.70], [32, -0.22]],
      stD: 80, st: -0.10, td: 235, t: -1.15 }) };

  /* 三束支阻滯＝RBBB ＋ 一條分支阻滯 ＋ 第一度房室阻滯：
     三個特徵分散在不同導程，故給兩條（V1 看 RBBB、III 看 LAFB），PR 一律 300 ms。 */
  SPEC['trifasc-v1']  = { f: rhythm(1000, 3, 60, { p: 0.12, pr: 300, qrs: [[26, 0.36], [46, -0.30, 'r'], [68, 0.85, 'r']], stD: 55, td: 150, t: -0.24 }) };
  SPEC['trifasc-iii'] = { f: rhythm(1000, 3, 60, { p: 0.10, pr: 300, qrs: [[24, 0.18], [50, -0.85]], stD: 80, td: 150, t: 0.10 }) };

  /* 短 QT：ST 段幾乎不存在（T 波直接接在 S 波上），T 高聳、基底窄。
     QT ≈ 285 ms、QTc ≈ 320——比 qt-normal 的 stD 90／td 160 短了一大截。 */
  SPEC['qt-short'] = { f: rhythm(800, 3, 60, { stD: 15, td: 165, t: 0.58 }) };

  /* 高血鎂：PR 與 QRS 一起被拖長、心跳慢；與高血鉀的差別在 T 波不高尖。 */
  SPEC.hypermg = { f: rhythm(1100, 3, 60, { p: 0.12, pd: 110, pr: 265,
      qrs: [[26, -0.08], [64, 0.85, 'r'], [50, -0.20, 'r']], stD: 80, td: 190, t: 0.24 }) };
