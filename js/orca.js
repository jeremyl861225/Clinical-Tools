...
(function () {
  ...
  /* 腹白：下顎一路到肛門，近尾處往上翹一撇（虎鯨側面最好認的一筆）。
     頭端的控制點必須跟額隆一樣**密集鋪在 u<0.09**：腹白的上緣就是
     y = botH(u) − ventH(u)，額隆的曲率全在那一段，控制點若從 0 直接跳到 0.04，
     單調三次只能用一條近乎直線接過去，下巴前緣就折成一個角（實測站點折角 40.6°、
     連續曲線的曲率半徑只有 3.3 px）。現在的取值是「ventH ≈ 0.45～0.55 × botH」，
     腹白的邊界因此**貼著頭的輪廓走**，把額隆的圓弧照抄過來（折角 26°、半徑 7.0 px）。 */
  var VK   = [0, .009, .0225, .045, .0675, .09, .17, .30, .52, .63, .70, .78, .86, 1];
  var VH   = [.009, .163, .259, .374, .452, .492, .539, .46, .52, .74, .58, .26, .04, 0];
  /* 側向半寬：胸圍處近乎圓，尾柄側扁。**吻端也要照圓帽收到零**——先前 u=0 是
     0.40，配上 topH/botH 的 0.02 等於一片 20:1 的橫刀片（L=430 時寬 18.6 px、
     高 0.93 px）。純側面看不出來（Z 乘上 cosθ=0），但**斜著看**那片刀片會投影成
     一條十來個像素的平舌頭，腹白的上緣就踩上去、再一階掉下來，成為嘴前方一個
     六十幾度的折角。控制點與額隆同一組，值取 0.72·√(1−(1−u/0.10)²)；u≥0.10
     之後與舊值最多差 0.12 px，頭以後的輪廓完全沒動。 */
  var WK   = [0, .009, .0225, .045, .0675, .09, .22, .35, .50, .66, .80, .90, 1];
  var WV   = [0, .2985, .455, .6013, .6809, .7164, .80, .78, .66, .48, .30, .20, .13];
  ...
  /* ---- 嘴是**真的透空**的 ----
     暗色那一圈從 KB+1 繞到 KN−1、腹白那一圈從 0 到 KB，兩圈的兩個接口
     （KB↔KB+1 與 KN−1↔0）刻意不接面片：下顎轉開之後，上下顎之間看到的就是
     背景。這是使用者要的（張口要能看穿，跟真的一樣），**不是漏畫**。
     曾經補過一次口腔內側（把縫填成暗色），使用者要求改回透空，別再補。 */
  ...
  function pecBase(side, out) {
    var u = D.pecU;
    return proj(bx(u), spine(u) + botH(u) * .74, halfW(u) * .55 * side, out);
  }
  function pecFin(g, side) {
    var u = D.pecU, w = halfW(u), i, a, sc, n = 0;
    /* 鰭根的高度 0.74×botH 是**壓在腹白裡面**的，不是貼在黑白交界上。
       先前是 0.40，剛好落在交界的上方一點點：胸鰭的前緣（往下後方掠 34°）
       就在交界處鑽出來，把腹白的上緣咬出一個 32° 的尖角——使用者說的
       「腹部白色區塊在嘴巴附近呈現折角」。鰭根沉到交界下方之後，腹白的上緣
       是一條從尾柄一路到下巴的連續曲線，胸鰭則單純是一片壓在白肚子上的黑板子。 */
    var bX = bx(u), bY = spine(u) + botH(u) * .74, bZ = w * .55 * side;   // 鰭根埋進體內
    pecAxis(side, _ax);
